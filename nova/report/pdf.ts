 ...
 * script. Same approach as SAGE. Optional: only used by the Full Prospect Report
 * when format=pdf is requested and Python/Playwright is installed.
 */
export async function htmlToPdf(html: string): Promise<Buffer> {
  const dir = await mkdtemp(join(tmpdir(), 'nova-pdf-'));
...
function runPython(args: string[]): Promise<void> {
  return new Promise((resolve, reject) => {
    const child = spawn(config.python.bin, [PDF_SCRIPT, ...args], { env: { ...process.env } });
    let errOut = '';
    const timer = setTimeout(() => { child.kill(); reject(new Error('PDF render timed out')); }, 120000);
    child.stderr.on('data', (d) => (errOut += d));
    child.on('error', (e) => { clearTimeout(timer); reject(new Error(`cannot run python (${config.python.bin}): ${e.message}`)); });
    child.on('close', (code) => {
      clearTimeout(timer);
      if (code === 0) resolve();
